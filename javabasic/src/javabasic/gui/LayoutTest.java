...
public class LayoutTest {
	...
	public static void main(String[] args) {
		...
		//1. FlowLayout
		JPanel flowLayoutPanel = new JPanel(new FlowLayout()); //안적어도 기본값은 flowlayout
		for (int i=1; i<=100; i++) {
		...
		//3. GridLayout
		JPanel gridLayoutPanel = new JPanel(new GridLayout(2,3)); //행, 열
		for (int i=1; i<=30; i++) {
		...
		for (int i=1; i<=15; i++) {
			cardLayoutPanel.add(new JLabel("카드" +i, SwingConstants.CENTER),
					"Card" + i);
		}
	    ...
	    gbc.gridy = 1;
	    gbc.gridheight = 2;
	    gridBagLayoutPanel.add(new JButton("버튼1(0,1, 높이2)"), gbc);
	    ...
	    gbc.gridx = 1;
	    gbc.gridy = 2;
	    gridBagLayoutPanel.add(new JButton("버튼3(1,2)"), gbc);
	    
	    tabbedPane.addTab("GridBagLayout", gridBagLayoutPanel);

	    //6. BoxLayout
	    JPanel boxLayoutPanel = new JPanel();
	    boxLayoutPanel.setLayout(new BoxLayout(boxLayoutPanel, BoxLayout.Y_AXIS)); //Y축(X축도 있음)
